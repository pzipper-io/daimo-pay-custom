...
export default function DemoExtendDeposit() {
  ...
  const handleWaitingPayment = useCallback((params: WaitingDepositAddressParams) => {
    console.log("****等待付款參數****:", params);
  }, []);
  ...
  useEffect(() => {
    ...
    const forcePayToAddressCode = config.forcePayToAddress
      ? `
  onWaitingPayment={handleWaitingPayment}
  forcePayToAddress={{
...
const handleWaitingPayment = (params) => {
  console.log("等待付款參數:", params);
  // 在這裡處理等待付款的邏輯
};
...
<DaimoPayButton
  ...
  intent="Deposit"${forcePayToAddressCode}
/>`;
  ...
  return (
    <Container className="max-w-4xl mx-auto p-6">
      <Text className="text-lg text-gray-700 mb-4">
        這個示例展示了如何使用擴展存款功能，包括等待付款回調和強制付款方法選擇。
      </Text>
            ...
            {waitingPaymentData && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 w-full max-w-md">
                ...
                <pre className="text-xs text-blue-600 overflow-auto">
                  {JSON.stringify(waitingPaymentData, null, 2)}
                </pre>
              </div>
            )}
...
// 自訂設定面板組件
function ExtendDepositConfigPanel({
  isOpen,
  onClose,
  onConfirm,
  defaultConfig,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <Text className="text-lg font-semibold mb-4">設定擴展存款</Text>
